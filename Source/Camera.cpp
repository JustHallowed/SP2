...
void Camera::Update(double dt) { //Update cam
	if (!keys.paused)
	{
		...
		if (Application::IsKeyPressed(keys.currentKey_left) - Application::IsKeyPressed(keys.currentKey_right)) { //Move cam left or right
			Vector3 dir = target - pos, front = dir.Normalized(), right = front.Cross(up).Normalized();
			right.y = 0;
			if (mode == MODE::FOCUS) {
				float yaw = float(float(Application::IsKeyPressed(keys.currentKey_left) - Application::IsKeyPressed(keys.currentKey_right)) * -focusSpd * dt);
				Mtx44 rotation;
				...
				pos += float(Application::IsKeyPressed(keys.currentKey_left) - Application::IsKeyPressed(keys.currentKey_right)) * float(-freeSpd * dt) * right;
				target += float(Application::IsKeyPressed(keys.currentKey_left) - Application::IsKeyPressed(keys.currentKey_right)) * float(-freeSpd * dt) * right;
			}
		}
		...
		if (Application::IsKeyPressed(keys.currentKey_front) || (leftMouse && !rightMouse)) { //Move cam forward or towards the target
			Vector3 dir = target - pos, front = dir.Normalized(), right = front.Cross(up).Normalized();
			right.y = 0;
			if (mode == MODE::FOCUS) {
				...
				if (Application::IsKeyPressed(keys.currentKey_front)) {
					front.y = 0;
				}
		...
		if (Application::IsKeyPressed(keys.currentKey_back) || (rightMouse && !leftMouse)) { //Move cam backward or away from the target
			Vector3 dir = target - pos, front = dir.Normalized(), right = front.Cross(up).Normalized();
			right.y = 0;
			if (mode == MODE::FOCUS) {
				...
				if (Application::IsKeyPressed(keys.currentKey_back)) {
					front.y = 0;
				}
				pos += float(-freeSpd * dt) * front;
				target += float(-freeSpd * dt) * front;
	...
	if (Application::IsKeyPressed('T')) //change controls
	{
		keys.onKeyChange = true;
		if (Application::IsKeyPressed(keys.currentKey_left))
		{
			keys.paused = true;
			while (keys.paused == true)
			{
				//char ch;
				keys.purpose = ChangeControl::PURPOSE::LEFT;
				//ch = _getch();
				keys.setnewKey('H');
				keys.paused = false;
		...
		else if (Application::IsKeyPressed(keys.currentKey_right))
		{
			keys.paused = true;
			while (keys.paused == true)
			{
				//char ch;
				keys.purpose = ChangeControl::PURPOSE::RIGHT;
				//ch = _getch();
				keys.setnewKey('K');
				keys.paused = false;
		...
		else if (Application::IsKeyPressed(keys.currentKey_front))
		{
			keys.paused = true;
			while (keys.paused == true)
			{
				//char ch;
				keys.purpose = ChangeControl::PURPOSE::FORWARD;
				//ch = _getch();
				keys.setnewKey('U');
				keys.paused = false;
		...
		else if (Application::IsKeyPressed(keys.currentKey_back))
		{
			keys.paused = true;
			while (keys.paused == true)
			{
				//char ch;
				keys.purpose = ChangeControl::PURPOSE::BACKWARD;
				//ch = _getch();
				keys.setnewKey('J');
				keys.paused = false;
